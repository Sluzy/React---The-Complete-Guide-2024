...
export default function EditProject() {
    ...
    const { selectedProject, removeTask, handleAddTask, deleteProject, updateProjectDescription } = useContext(ProjectContext);

    const [newTaskInput, setNewTaskInput] = useState("");
    const [showAlert, setShowAlert] = useState(false);

    const [isEditable, setIsEditable] = useState(false);
    const [editableDescription, setEditableDescription] = useState(selectedProject.description);
    ...
    function addTaskHandler() {
        if (newTaskInput.trim() === "") {
            setShowAlert(true);
            setTimeout(() => setShowAlert(false), 3000); // Hides alert after 3 seconds
            return;
        }
        handleAddTask(newTaskInput, clearInput);
    }
    ...
    function deleteHandler() {
        deleteProject(selectedProject.title);
        closeDeleteModal();
    }
    ...
    function toggleEdit() {
        if (isEditable) {
            ...
            updateProjectDescription(editableDescription, selectedProject.title);
        } else {
    ...
    return (
        <>
            <div className="bg-white shadow-md rounded-lg p-6 w-full md:w-3/5 lg:w-2/5">
                ...
                <p className="text-md text-gray-600 mb-4">{selectedProject.date}</p>
                <textarea
